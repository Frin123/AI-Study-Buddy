...
# 1. UPLOAD SECTION
uploaded_file = st.file_uploader("Upload your study notes here", type=["png", "jpg", "jpeg", "pdf"])
content_to_analyze = []
...
if uploaded_file:
    ...
    if uploaded_file.type == "application/pdf":
        with st.spinner("Processing PDF pages..."):
            try:
                ...
                # CHANGE THE PATH BELOW to where you extracted Poppler
                images = convert_from_bytes(
                    uploaded_file.read(), 
                    poppler_path="C:\Users\USER\poppler-25.12.0\Library\bin" 
                )
                
                # We'll take the first 5 pages to keep things fast
                content_to_analyze = images[:5]
                st.success(f"✅ Loaded {len(content_to_analyze)} pages for analysis.")
            except Exception as e:
    ...
    if uploaded_file.type == "application/pdf":
        ...
        with st.spinner("Reading PDF..."):
            ...
            # For PDFs, our 'content' is just the extracted text
            content_to_analyze = [text_content]
            st.success(f"Successfully loaded {len(pdf_reader.pages)} pages!")
    
    else:
        # Handle Image (Your existing logic)
        img = Image.open(uploaded_file)
        content_to_analyze = [img]

        # --- 1. QUALITY CHECK LOGIC ---
        # We only run this if we haven't checked this specific image yet
        if "quality_warning" not in st.session_state or st.session_state.last_image_name != uploaded_file.name:
            with st.spinner("🔍 Checking image clarity..."):
                try:
                    # Use a fast prompt to check readability
                    check_prompt = "Analyze this image quality. Is the text clear and readable? Answer ONLY 'PASS' if it's good, or 'FAIL: [reason]' if it is blurry, dark, or unreadable."
                
                    # We use the 'Fast' model for this to save time
                    quality_response = client.models.generate_content(
                    model="gemini-2.0-flash", 
                    contents=[check_prompt, img]
                    )
                
                    st.session_state.quality_warning = quality_response.text
                except Exception:
                    st.session_state.quality_warning = "PASS" # Default to pass if check fails

        # --- 2. DISPLAY WARNING IF NEEDED ---
        if "FAIL" in st.session_state.quality_warning:
            st.warning(f"⚠️ **Potential Issue Detected:** {st.session_state.quality_warning.replace('FAIL:', '')}")
            st.info("You can still try to analyze it, but results may be inaccurate. For best results, ensure good lighting and focus.")
        ...
        if st.button("Analyze Everything"):
            with st.spinner("Processing..."):
                try:
                    ...
                    #Combine prompt string with the content list
                    # This works whether content_to_analyze has 1 image or 5 PDF pages
                    full_contents = [extraction_prompt] + content_to_analyze
            ...
            if user_question:
                with st.spinner("Thinking..."):
                    try:
                        ...
                        # 3. Call Model
                        # Combine the user question prompt with the prepared content
                        full_contents = [qa_prompt] + content_to_analyze
                        response = client.models.generate_content(
                            model=selected_model,
                            config=qa_config,
                            contents=full_contents
